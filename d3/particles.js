...
function createParticle(x, y) {
  let particle = {x, y};
  particle.id = id;
  particle.theta = randomRadians();
  particle.vx = Math.sin(particle.theta);
  particle.vy = Math.cos(particle.theta);
  particle.r = randomParticleRadius();
  particle.color = randomTurboColor();
...
function moveParticle(particle) {
  ...
  movedParticle.id = particle.id;
  movedParticle.theta = particle.theta + randomDeltaTheta();
  movedParticle.vx = particle.vx + Math.sin(particle.theta);
...
function randomTurboColor() {
  return colorArray[Math.floor(Math.random() * colorArray.length)];
}

d3.interval(() => {
  particles = particles.map(particle => moveParticle(particle));
